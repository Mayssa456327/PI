...
class SponsorController extends AbstractController
{
    ...
    #[Route('{idevent}/new', name: 'app_sponsor_new', methods: ['GET', 'POST'])]
public function new(Request $request, EntityManagerInterface $entityManager, Evenement $idevent): Response
{
    $sponsor = new Sponsor();
    $sponsor->setEvenement($idevent);
    
    $form = $this->createForm(SponsorType::class, $sponsor);
    $form->handleRequest($request);

    if ($form->isSubmitted() && $form->isValid()) {
        // Récupérer le montant du budget du sponsor à partir du formulaire
        $sponsorBudget = $form->get('budget')->getData();
        
        // Mettre à jour le budget de l'événement
        $newBudget = $idevent->getBudget() - $sponsorBudget;
        $idevent->setBudget($newBudget);
